...
def main():
    # Prepare
    ascii_art = {}
    art_files = ["castle.txt", "chimera.txt", "golem.txt", "treasure.txt", "troll.txt"]
    for art in art_files:
        art_name = art.replace(".txt", "")
        get_art(ascii_art, art, art_name)

    # Tutorial
    print("Welcome to a little game, that allows you to take a role of an adventurer, on a search for treasure.",
    ...
    print("First, you need to pick a class. Each have their own set of skills that you have to utilize to defeat the monsters",
          "You can be either Warrior or Rogue")
    player = get_class()
    showing_tutorial()

    # Level 1
    print("You are in the woods, on a path that leads to the castle. The trees shake around you and the troll blocks your path.")
    troll = Troll()
    ...
    if player.life:
        ...
        print(leveled)
        print("Having defeated the troll, you proceed into the castle. The halls are long and narrow, and eventually you bump into the door,",
              "guarded by the terrifying golem-watcher.")
        proceeding = [inquirer.List("proceed", message="Press enter when you are ready to continue", choices=["Continue"])]
    ...
    if player.life:
        leveled = level_up(player)
        print(leveled)
        print("The golem falls and you proceed further. You feel that you are close - the tresuary is right in front of you,",
...
def get_class():
    ...
    while True:
        chosen_class = input("Type 'warrior' or 'rogue' to pick your class: ").strip().lower()
...
def showing_tutorial():
    tut_question = [
                inquirer.List("tutorial", message="Would you like a tutorial on combat mechanics?", choices=["Yes", "No"])
            ]
    show_tutorial = inquirer.prompt(tut_question)
    if show_tutorial["tutorial"] == "Yes":
        rprint("The combat is turn-based. You go first, so you have an advantage. You win if enemy's health drops to 0.\n",
        ...
        proceeding = [
                    inquirer.List("proceed", message="Press enter when you are ready to start the game", choices=["Continue"])
                ]
        inquirer.prompt(proceeding)
...
def fight(monster, player, ascii):
    ...
    while monster.hp > 0 and player.hp > 0:
        ...
        monster_elem_dmg = announce_elem_dmg(monster, f"{monster.name} is", monster.name)
        if monster_elem_dmg:
...
def congrats():
    return "You overcame all your trials and found the treasure! Never again will you have to worry about the money again, and your future looks as bright as ever."


def announce_class(class_chosen):
    return f"You have chosen the {class_chosen}"
